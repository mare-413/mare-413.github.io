...
Board.prototype.inputs = function () {
   ...
   inputs.forEach((v, i) => inputs[i] = v == Infinity ? 0 : v);
   //-------- WALL BLOCK ---------
   inputs.push(h.x / grid);
   inputs.push(((grid - 1) - h.x) / grid);
   inputs.push(h.y / grid);
   inputs.push(((grid - 1) - h.y) / grid);
   //-------- FOOD BLOCK ---------
   var [fx, fy] = [h.x - this.food.x, h.y - this.food.y];
   inputs.push((fy > 0 ? fy : 0) / (grid / 2));
   inputs.push((fx < 0 ? Math.abs(fx) : 0) / (grid / 2));
   inputs.push((fy < 0 ? Math.abs(fy) : 0) / (grid / 2));
   inputs.push((fx > 0 ? fx : 0) / (grid / 2));
   return inputs;
}

Board.prototype.evaluate = function () {
   this.fitness = this.score < 8 ? floor(this.lifetime * (this.score * 2 * this.score)) : floor(this.lifetime / 3 * (this.score * this.score));
   return this.fitness;
}
